...
import { ScriptNodeContent } from "./canvas-script-node-content";
...
function LoadingContent({ theme, node }: Pick<NodeContentRendererProps, "theme"> & { node?: CanvasNodeData }) {
    const progress = node?.metadata?.progress as number | undefined;
            ...
            {hasProgress ? (
                <div className="relative size-10">
                    <svg className="size-10 -rotate-90" viewBox="0 0 36 36">
                        <circle cx="18" cy="18" r="16" fill="none" stroke={theme.node.stroke} strokeWidth="2.5" />
                        <circle cx="18" cy="18" r="16" fill="none" stroke={theme.node.activeStroke} strokeWidth="2.5" strokeDasharray={`${progress} 100`} strokeLinecap="round" />
                    </svg>
                    <span className="absolute inset-0 flex items-center justify-center text-[8px] font-medium">{progress}%</span>
                </div>
            ) : (
    ...
    if (!props.node.metadata?.content && props.isBatchRoot) {
        const content =
            props.node.metadata?.status === "loading" ? (
                <LoadingContent theme={props.theme} node={props.node} />
...
function ConnectionHandleDot({ side, visible, onMouseDown }: { side: "left" | "right"; visible: boolean; onMouseDown: (event: React.MouseEvent | React.PointerEvent) => void }) {
    const theme = useCanvasTheme();

    return (
        <div
            className={`canvas-connection-handle absolute top-1/2 z-30 flex size-12 -translate-y-1/2 cursor-crosshair items-center justify-center transition-opacity duration-150 ${
        ...
        >
            <div className="canvas-connection-handle-dot size-3 rounded-full border-2 transition-all hover:scale-125" style={{ background: theme.node.panel, borderColor: theme.node.muted, boxShadow: visible ? `0 0 12px ${theme.node.activeStroke}66` : undefined }} />
        </div>
    );
}
