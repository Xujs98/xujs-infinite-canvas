...
export function CanvasLocalAgentPanel({ snapshot, canUndoOps, collapsed, embedded, onApplyOps, onUndoOps }: { snapshot: CanvasAgentSnapshot; canUndoOps: boolean; collapsed?: boolean; embedded?: boolean; onApplyOps: (ops: CanvasAgentOp[]) => unknown; onUndoOps: () => CanvasAgentSnapshot | null }) {
    ...
    useEffect(() => {
        ...
        const mode = (typeof window !== "undefined" ? localStorage.getItem("canvas-agent-mode") : null) || "website";
        if (mode !== "website") return;
        let cancelled = false;
        (async () => {
            try {
                const res = await fetch("/api/agent/status");
                const data = (await res.json().catch(() => ({}))) as { running?: boolean; token?: string };
                if (cancelled) return;
                if (!data.running || !data.token) return;
                setAgentState({ url: "/api/agent", token: data.token, enabled: true, connected: false, activity: "连接中", connectError: "", activeTab: "chat" });
            } catch {}
        })();
        return () => { cancelled = true; };
    }, [collapsed, enabled, connected, setAgentState]);
        ...
        <>
            ...
            {activeTab === "setup" ? (
                <AgentConnectView
                    ...
                    onConnectWebsite={(websiteUrl, websiteToken) => setAgentState({ url: websiteUrl, token: websiteToken, enabled: true, connected: false, activity: "连接中", connectError: "", activeTab: "chat" })}
                />
            ) : activeTab === "history" ? (
                <AgentHistoryView
                    theme={theme}
...
function AgentConnectView({ theme, url, token, enabled, connected, activity, connectError, onUrlChange, onTokenChange, onToggleEnabled, onConnectWebsite }: { theme: (typeof canvasThemes)[keyof typeof canvasThemes]; url: string; token: string; enabled: boolean; connected: boolean; activity: string; connectError: string; onUrlChange: (value: string) => void; onTokenChange: (value: string) => void; onToggleEnabled: () => void; onConnectWebsite: (url: string, token: string) => void }) {
    const { message } = App.useApp();
    const [mode, setMode] = useState<"website" | "local">(() => (typeof window !== "undefined" ? (localStorage.getItem("canvas-agent-mode") as "website" | "local") || "website" : "website"));
    const [websiteLoading, setWebsiteLoading] = useState(false);
    const statusText = connectError ? "连接失败" : connected ? activity : enabled ? "连接中" : "未连接";
    const statusColor = connectError ? "#dc2626" : connected ? "#16a34a" : enabled ? "#d97706" : theme.node.muted;
    const websiteUrl = typeof window !== "undefined" ? `${window.location.origin}/api/agent` : "/api/agent";
    const copyCommand = (command: string) => {
        copyToClipboard(command);
        message.success("命令已复制");
    };
    const handleModeChange = (value: "website" | "local") => {
        setMode(value);
        localStorage.setItem("canvas-agent-mode", value);
    };
    const connectWebsite = async () => {
        ...
        try {
            const res = await fetch("/api/agent/status");
            const data = (await res.json().catch(() => ({}))) as { running?: boolean; token?: string; error?: string };
            if (!data.running) {
                message.error(data.error || "网站 Agent 服务未运行");
                return;
            }
            if (!data.token) {
                const text = "未获取到 Agent Token";
                message.error(text);
                return;
            }
            onConnectWebsite(websiteUrl, data.token);
        } catch {
            message.error("获取网站 Agent 状态失败");
        } finally {
        ...
        <div className="thin-scrollbar min-h-0 flex-1 overflow-y-auto p-4">
            <div className="space-y-4">
                <Segmented
                    ...
                    size="small"
                    value={mode}
                    onChange={(value) => handleModeChange(value as "website" | "local")}
                    options={[
                ...
                {mode === "website" ? (
                    <>
                        <div>
                            <div className="text-base font-semibold leading-6">网站 Agent</div>
                            ...
                            <div className="flex flex-wrap items-start justify-between gap-3">
                                <div className="min-w-0 flex-1">
                                    <div className="flex min-w-0 items-center gap-2">
                                        <span className="shrink-0 text-sm font-medium leading-5">网站连接</span>
                                ...
                                <Button className="!h-8 !px-3" type={enabled ? "default" : "primary"} icon={websiteLoading ? <LoaderCircle className="size-4 animate-spin" /> : <PlugZap className="size-4" />} loading={websiteLoading} disabled={websiteLoading} onClick={connectWebsite}>
                                    {enabled ? "断开" : "连接"}
                                </Button>
                            </div>
                            {connectError ? (
