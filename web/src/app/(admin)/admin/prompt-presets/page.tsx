  ...
  return (
    <div style={{ padding: "24px 28px" }}>
      <div style={{ marginBottom: 20 }}>
        <Typography.Title level={4} style={{ margin: 0, fontSize: 20, fontWeight: 600 }}>提示词预设</Typography.Title>
        <Typography.Text type="secondary" style={{ fontSize: 13 }}>管理提供给 app 同步的提示词预设</Typography.Text>
      </div>
            ...
            <Row gutter={16} align="bottom">
              <Col flex="360px">
                <Form.Item label="关键词">
                  <Input.Search
                    value={keywordText}
                    placeholder="搜索预设名称或提示词"
                    allowClear
                    enterButton={<SearchOutlined />}
                    onSearch={() => searchPresets(keywordText)}
              ...
              <Col flex="none">
                <Form.Item>
                  <Space>
                    ...
                    <Button type="primary" icon={<ReloadOutlined />} onClick={() => searchPresets(keywordText)}>查询</Button>
                  </Space>
                </Form.Item>
              </Col>
            </Row>
          ...
          toolBarRender={() => [
            selectedPresetIds.length > 0 && (
              <Popconfirm key="batch-delete" title={`确定删除选中的 ${selectedPresetIds.length} 条预设？`} onConfirm={() => void handleBatchDelete()} okButtonProps={{ danger: true }}>
                <Button danger icon={<DeleteOutlined />}>批量删除 {selectedPresetIds.length}</Button>
              </Popconfirm>
            ),
      ...
      <Modal
        title={editingPreset?.id ? "编辑预设" : "新增预设"}
