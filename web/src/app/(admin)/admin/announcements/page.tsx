...
export default function AdminAnnouncementsPage() {
    ...
    const columns: ProColumns<AdminAnnouncement>[] = [
        ...
        },
        {
            ...
            width: 80,
            align: "center",
            render: (_, item) => <Switch size="small" checked={item.pinned} onChange={() => handleTogglePinned(item)} />,
        },
        {
            title: "状态",
            dataIndex: "status",
    ...
    return (
        <div className="admin-data-page">
                ...
                <Card className="admin-filter-card" variant="borderless">
                    <Form layout="vertical">
                        <Row gutter={16} align="bottom">
                            <Col flex="360px">
                                <Form.Item label="关键词">
                                    <Input value={keywordText} placeholder="搜索公告标题或内容" allowClear onPressEnter={() => searchAnnouncements(keywordText)} onChange={(e) => setKeywordText(e.target.value)} />
                                </Form.Item>
                            </Col>
                            <Col flex="160px">
                                <Form.Item label="状态">
                                    <Select value={status || undefined} placeholder="全部状态" allowClear options={statusOptions} onChange={(value) => changeStatus(value || "")} />
                                </Form.Item>
                            </Col>
                            <Col flex="none">
                                <Form.Item>
                                    <Space>
                                        ...
                                        <Button type="primary" icon={<SearchOutlined />} onClick={() => searchAnnouncements(keywordText)}>
                                            查询
                                        </Button>
                                    </Space>
                                </Form.Item>
                    ...
                    toolBarRender={() =>
                        [
                            selectedRowKeys.length > 0 && (
                                <Space key="batch">
                                    <Popconfirm title={`确定批量置顶选中的 ${selectedRowKeys.length} 条公告？`} onConfirm={() => handleBatchPinned(true)}>
                                        <Button icon={<PushpinOutlined />}>批量置顶</Button>
                                    </Popconfirm>
                                    <Popconfirm title={`确定取消置顶选中的 ${selectedRowKeys.length} 条公告？`} onConfirm={() => handleBatchPinned(false)}>
                                        <Button>取消置顶</Button>
                                    </Popconfirm>
                                    <Popconfirm title={`确定删除选中的 ${selectedRowKeys.length} 条公告？`} onConfirm={handleBatchDelete} okButtonProps={{ danger: true }}>
            ...
            {/* 创建/编辑公告 */}
            <Modal title={editingAnnouncement?.id ? "编辑公告" : "创建公告"} open={!!editingAnnouncement} width={640} onCancel={() => setEditingAnnouncement(null)} onOk={() => void handleSave()} okText="保存" cancelText="取消" destroyOnHidden>
            ...
            {/* 删除确认 */}
            <Modal title="删除公告" open={!!deletingAnnouncement} onCancel={() => setDeletingAnnouncement(null)} onOk={() => void handleDelete()} okText="删除" cancelText="取消" okButtonProps={{ danger: true }}>
