...
export default function AdminSettingsPage() {
    ...
    const openChannelDrawer = (index: number | null) => {
        ...
        const extraHeadersList = channel.extraHeaders ? Object.entries(channel.extraHeaders).map(([key, value]) => ({ key, value })) : [];
            ...
            <div className="admin-config-inner mx-auto max-w-[1200px]">
                ...
                <div className="admin-config-toolbar sticky top-0 z-50 mb-7 flex items-center justify-between rounded-2xl border border-gray-100 bg-white/95 px-5 py-3 shadow-sm backdrop-blur-sm">
                    <div ref={navRef} className="admin-config-tabs relative flex items-center gap-1 rounded-2xl border border-gray-100 bg-gray-50 p-1.5">
                        ...
                        {settingsTabs.map((tab) => (
                            <button
                                key={tab.key}
                                ref={(el) => {
                                    if (el) tabBtnRefs.current.set(tab.key, el);
                                }}
                    ...
                    <Space>
                        ...
                        <Button type="primary" icon={<SaveOutlined />} loading={isSaving} onClick={() => void saveSettings()} className="!rounded-lg">
                            保存设置
                        </Button>
                    </Space>
                </div>
                ...
                <Flex vertical gap={16}>
                    <Card variant="borderless" className="admin-config-card !rounded-xl !border-gray-100 !shadow-sm">
                                            ...
                                            <Table
                                                ...
                                                columns={[
                                                    { title: "模型", dataIndex: "model" },
                                                    {
                                                        ...
                                                        dataIndex: "alias",
                                                        width: 200,
                                                        render: (_, item) => <Input placeholder="留空显示原名" value={item.alias} onChange={(e) => setModelCost(form, setModelCosts, item.model, item.credits, e.target.value)} />,
                                                    },
                                                    {
                                                        title: "每次调用扣除",
                                                        dataIndex: "credits",
                                    ...
                                    {channelMediaType === "image" && (
                                        <Col span={8}>
                                            <Form.Item name="responseFormat" label="响应格式" help="App 端解析响应的方式">
                                                <Select
                                                    allowClear
                                                    placeholder="默认 openai-images"
                                                    options={[
                                                        { label: "OpenAI Images", value: "openai-images" },
                                                        { label: "URL 数组", value: "url-array" },
                            ...
                            {channelMediaType === "video" && (
                                <div className={channelSectionClass}>
                                    ...
                                    <Row gutter={16}>
                                        ...
                                        <Col span={6}>
                                            <Form.Item name={["videoConfig", "modelField"]} label="模型字段">
                                                <Input placeholder="model" />
                                            </Form.Item>
                                        </Col>
                                        <Col span={6}>
                                            <Form.Item name={["videoConfig", "promptField"]} label="提示词字段">
                                                <Input placeholder="prompt" />
                                            </Form.Item>
                                        </Col>
                                        <Col span={6}>
                                            <Form.Item name={["videoConfig", "sizeField"]} label="尺寸字段">
                                                <Input placeholder="size" />
                                            </Form.Item>
                                        </Col>
                                        <Col span={6}>
                                            <Form.Item name={["videoConfig", "secondsField"]} label="时长字段">
                                                <Input placeholder="seconds" />
                                            </Form.Item>
                                        </Col>
                                        <Col span={6}>
                                            <Form.Item name={["videoConfig", "aspectRatioField"]} label="比例字段">
                                                <Input placeholder="aspect_ratio" />
                                            </Form.Item>
                                        </Col>
                                        <Col span={6}>
                                            <Form.Item name={["videoConfig", "referenceImagesField"]} label="参考图字段">
                                                <Input placeholder="images" />
                                            </Form.Item>
                                        </Col>
                                        <Col span={6}>
                                            <Form.Item name={["videoConfig", "firstFrameField"]} label="首帧字段">
                                                <Input placeholder="可选" />
                                            </Form.Item>
                                        </Col>
                                        <Col span={6}>
                                            <Form.Item name={["videoConfig", "lastFrameField"]} label="尾帧字段">
                                                <Input placeholder="可选" />
                                            </Form.Item>
                                        </Col>
                                        <Col span={12}>
                                    ...
                                    <div className={channelNestedSectionClass}>
                                        ...
                                        <Row gutter={16}>
                                            ...
                                            <Col span={6}>
                                                <Form.Item name={["videoConfig", "imageInput", "min"]} label="最少数量">
                                                    <Input type="number" placeholder="0" />
                                                </Form.Item>
                                            </Col>
                                            <Col span={6}>
                                                <Form.Item name={["videoConfig", "imageInput", "max"]} label="最多数量">
                                        ...
                                        <Row gutter={16}>
                                            ...
                                            <Col span={6}>
                                                <Form.Item name={["videoConfig", "videoInput", "min"]} label="最少数量">
                                                    <Input type="number" placeholder="0" />
                                                </Form.Item>
                                            </Col>
                                            <Col span={6}>
                                                <Form.Item name={["videoConfig", "videoInput", "max"]} label="最多数量">
                                                    <Input type="number" placeholder="0" />
                                                </Form.Item>
                                            </Col>
                                            <Col span={6}>
                                                <Form.Item name={["videoConfig", "videoInput", "field"]} label="字段名">
                                        ...
                                        <Row gutter={16}>
                                            ...
                                            <Col span={6}>
                                                <Form.Item name={["videoConfig", "audioInput", "min"]} label="最少数量">
                                                    <Input type="number" placeholder="0" />
                                                </Form.Item>
                                            </Col>
                                            <Col span={6}>
                                                <Form.Item name={["videoConfig", "audioInput", "max"]} label="最多数量">
                                                    <Input type="number" placeholder="0" />
                                                </Form.Item>
                                            </Col>
                                            <Col span={6}>
                                                <Form.Item name={["videoConfig", "audioInput", "field"]} label="字段名">
                                                    <Input placeholder="audios" />
                                                </Form.Item>
                                            </Col>
                                        </Row>
                                    </div>
                    ...
                    >
                        <Flex vertical gap={14}>
                            <Flex gap={12} wrap>
                                <Input placeholder="搜索模型" allowClear value={modelSelectKeyword} onChange={(event) => setModelSelectKeyword(event.target.value)} style={{ flex: "1 1 260px" }} />
                    ...
                    >
                        <Flex vertical gap={12}>
                            ...
                            <Input placeholder="搜索模型..." allowClear value={testKeyword} onChange={(event) => setTestKeyword(event.target.value)} />
                            <Table
                                rowKey="model"
                                pagination={false}
                                scroll={{ y: 420 }}
...
function normalizeChannelFromForm(values: any): AdminModelChannel {
    const videoConfig = values.videoConfig
        ? {
              ...values.videoConfig,
              videoUrlPaths:
                  typeof values.videoConfig.videoUrlPaths === "string"
                      ? values.videoConfig.videoUrlPaths
                            .split(",")
                            .map((s: string) => s.trim())
                            .filter(Boolean)
                      : values.videoConfig.videoUrlPaths,
              pendingValues:
                  typeof values.videoConfig.pendingValues === "string"
                      ? values.videoConfig.pendingValues
                            .split(",")
                            .map((s: string) => s.trim())
                            .filter(Boolean)
                      : values.videoConfig.pendingValues,
              successValues:
                  typeof values.videoConfig.successValues === "string"
                      ? values.videoConfig.successValues
                            .split(",")
                            .map((s: string) => s.trim())
                            .filter(Boolean)
                      : values.videoConfig.successValues,
              failedValues:
                  typeof values.videoConfig.failedValues === "string"
                      ? values.videoConfig.failedValues
                            .split(",")
                            .map((s: string) => s.trim())
                            .filter(Boolean)
                      : values.videoConfig.failedValues,
              ...
              imageInput: values.videoConfig.imageInput
                  ? {
                        ...
                        roles:
                            typeof values.videoConfig.imageInput.roles === "string"
                                ? values.videoConfig.imageInput.roles
                                      .split(",")
                                      .map((s: string) => s.trim())
                                      .filter(Boolean)
                                : values.videoConfig.imageInput.roles,
    ...
    return {
        ...
        supportedResolutions:
            typeof values.supportedResolutions === "string"
                ? values.supportedResolutions
                      .split(",")
                      .map((s: string) => s.trim())
                      .filter(Boolean)
                : values.supportedResolutions,
