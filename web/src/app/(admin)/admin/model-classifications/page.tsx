...
export default function AdminModelClassificationsPage() {
    ...
    const renderConfigSummary = (item: AdminModelClassification) => {
        ...
        if ((item.capability === "text" || item.capability === "chat") && item.chatConfig) {
            return (
                <div className="mt-2 flex flex-wrap gap-1">
                    <Tag color="blue" className="text-xs">上下文 {item.chatConfig.contextWindow?.toLocaleString() ?? '-'} tokens</Tag>
                    <Tag color="blue" className="text-xs">最大输出 {item.chatConfig.maxOutputTokens?.toLocaleString() ?? '-'} tokens</Tag>
                    {item.chatConfig.supportsMultimodal && <Tag color="blue" className="text-xs">多模态</Tag>}
                    {item.chatConfig.description && <Tag color="blue" className="text-xs">{item.chatConfig.description}</Tag>}
                </div>
            );
        }
        return null;
            ...
            >
                <Form form={form} layout="vertical" className="mt-4">
                    ...
                    {(capability === "text" || capability === "chat") && (
                        <Card title={<span className="flex items-center gap-2"><FileTextOutlined className="text-blue-500" />对话/文本参数</span>} size="small" className="mb-4 border-blue-100 bg-blue-50/30">
                            <Row gutter={16}>
                                <Col span={8}>
