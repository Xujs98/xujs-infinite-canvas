...
function appendFlattenedPaths(value: unknown, currentPath: string, results: Array<{ path: string; label: string }>, visited: Set<unknown>, depth: number, maxDepth: number, limit: number) {
...
export default function AdminAITextAgentsPage() {
    const { agents, keyword, page, pageSize, total, isLoading, searchAgents, changePage, changePageSize, resetFilters, refreshAgents, saveAgent, deleteAgent, deleteAgents } = useAdminAITextAgents();
        ...
        <div className="admin-data-page">
            <div className="admin-page-title">
                ...
                <Typography.Text type="secondary" style={{ fontSize: 13 }}>
                    管理提供给 app 同步的 AI Agent 配置，后续可扩展图片、视频 Agent
                </Typography.Text>
            </div>
                ...
                <Card className="admin-filter-card" variant="borderless">
                    <Form layout="vertical">
                        <Row gutter={16} align="bottom">
                            <Col flex="360px">
                                <Form.Item label="关键词">
                                    <Input value={keywordText} placeholder="搜索名称、模型或提示词" allowClear onPressEnter={() => searchAgents(keywordText)} onChange={(event) => setKeywordText(event.target.value)} />
                                </Form.Item>
                            </Col>
                            <Col flex="none">
                                <Form.Item>
                                    <Space>
                                        ...
                                        <Button type="primary" icon={<SearchOutlined />} onClick={() => searchAgents(keywordText)}>
                                            查询
                                        </Button>
                                    </Space>
                                </Form.Item>
            ...
            <Modal title={editingAgent?.id ? "编辑 AI Agent" : "新增 AI Agent"} open={Boolean(editingAgent)} width={920} onCancel={() => setEditingAgent(null)} onOk={() => void handleSave()} okText="保存" cancelText="取消" destroyOnHidden>
                                        ...
                                        return (
                                            <Card key={field.key} size="small" style={{ background: "#fcfcfc" }}>
                                                ...
                                                <Row gutter={12} align="bottom">
                                                    ...
                                                    <Col span={3}>
                                                        <Button danger block onClick={() => remove(field.name)} disabled={fields.length <= 1}>
                                                            删除
                                                        </Button>
                                                    </Col>
                                                </Row>
                                        ...
                                        {fields.length > 0 ? (
                                            <div style={{ marginTop: 8 }}>
                                                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
                                                    ...
                                                    <Button size="small" onClick={localizeJsonFieldLabels}>
                                                        一键填入中文汉化
                                                    </Button>
                                                </div>
