...
- 任何代码、配置或资源变更都必须在同一个任务中同步更新对应文档；不能只改实现、不改文档。
- 每次任务完成前必须核对文档是否准确覆盖本次行为、配置项、接口和验证结果；未完成文档更新时不得宣称任务完成。
- 每次完成变更后都必须提交 Git 并推送 GitHub。推送失败时最多重试两次；连续三次失败要明确告知，并在下一次编码任务中优先补推。
- 提交时只暂存本次任务相关文件，保留用户已有的无关改动，不得顺手回滚或混入提交。
