...
+ [修复] 修复后台图片模型“批量提交并发数”和“支持自定义尺寸”未进入表单保存白名单、提交时总是回退默认值的问题。
+ [新增] 图片模型参数新增 1-20 的“批量提交并发数”，App 按渠道和模型隔离批量生图队列，并在上游接受提交后释放并发槽位。
+ [修复] SQLite 日志写入与算力点扣费并发时可能返回 `SQLITE_BUSY`，批量模型调用现在会安全排队写入。
+ [优化] 文本模型支持端到端 SSE 实时透传和保活，降低长篇 AI 脚本生成触发网关 524 超时的概率。
+ [新增] 新增卡密功能，支持管理员批量生成算力点/会员时长卡密，用户可在账户菜单中兑换。
+ [优化] 后台日志管理新增视频生成完成记录，详情面板支持查看响应 JSON 和视频预览。
